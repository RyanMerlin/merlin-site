...
The machine has the mirror-image problem.  Asked to fix its own reasoning with [nothing new to go on](https://arxiv.org/abs/2310.01798), a model often makes things worse, because the same blind spot that produced the error is the one doing the checking.  *Jie Huang* and colleagues showed exactly that: left to grade its own work with nothing new to check against, a model tends to get worse, not better.  The models that *do* reliably self-correct only learned how by being trained against an outside signal, which is the same point wearing a lab coat: correction needs something the blind channel didn't already have.
