...
Six agents, each a persistent [Claude Code](https://docs.anthropic.com/en/docs/claude-code) session, each scoped to a specific domain:
...
**Agents that survive infrastructure events without human intervention.**  If excalibur reboots, EdgePlane comes back up as a systemd service and restarts all six profiles automatically.  No SSH session, no manual attach, no checking which ones came back and which ones need a nudge.
