...
<li>AI agents create a new allocation layer between supply and human demand.  The scarce position is no longer visibility alone, but admission into the set an agent is willing and authorized to consider.</li>
<li>The economically important transitions are not listing or mention, but eligibility, retrieval, verification, recommendation, invocation, execution, and measured outcome.</li>
<li>Repeated machine selection leaves residue.  Successful tools become easier to retrieve, trust, authorize, and select again, creating a form of compounding attention capital.</li>
<li>Apparent agreement across many agents may reflect shared models, registries, indexes, defaults, or infrastructure rather than genuinely independent judgment.</li>
<li>Market power will increasingly belong to whoever controls candidate sets, permissions, defaults, and the evidence agents use to justify action.</li>
</ol>
</section>
...
![The machine-attention vote: an install is the human vote, an agent-initiated invocation is the machine vote, execution is delivery, and the measured outcome is the receipt.](./attention-vote-strip.svg)
...
![The compounding loop: an initial boost opens the loop at admission; the option enters the trusted set, gets acted on, and accumulates attention capital (logs, reviews, revenue), which lowers the cost of the next selection and makes future admission easier.  The original boost can vanish while the advantage stays.](./sticky-machine-reality-loop.svg)
...
![The correlated witness: a hundred agent processes drawing on one foundation model, one search index, one registry, and one capability compress into a single shared view, so their agreement inherits one shared blind spot.  A hundred processes at 0.2 error correlation act like roughly 4.8 independent observers.](./correlated-observers.svg)
