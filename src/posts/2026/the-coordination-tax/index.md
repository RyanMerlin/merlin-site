...
The next piece in this series: [Persistent sessions are the unit of agent work, not requests](/posts/persistent-sessions-unit-of-agent-work).  Why treating a model call as an RPC is the wrong abstraction for long-running agents, and what the supervisor loop actually looks like.